  ...
  class Program
  {
    ...
    async Task<int> OnExecute()
    {
      ...
      foreach (ProcessedResultsCollection collection in _results)
      {
        ...
        _logger.LogInformation($"Success: {collection.results.Count(x => x.result == Result.Success)}");
        _logger.LogInformation($"Unauthorized: {collection.results.Count(x => x.result == Result.Unauthorized)}");
        _logger.LogInformation($"Unprocessable: {collection.results.Count(x => x.result == Result.Unprocessable)}");
